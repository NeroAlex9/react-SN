...
export const getUsers = (activePage, totalUsersCount) => async (dispatch) => {
    dispatch(toggleIsFatching(true));
    const response = await usersApi.getUsers(activePage, totalUsersCount)
    dispatch(toggleIsFatching(false));
    dispatch(setUser(response.items));
    dispatch(pageSize(response.totalCount))
...
export const unFollowUser = (id) => async (dispatch) => {
    ...
    const response = await usersApi.unFollov(id)
...
export const followUser = (id) => async (dispatch) => {
    dispatch(toggleFollowingProgress(true, id))
    const response = await usersApi.follov(id)
